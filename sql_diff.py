...
DB_FILE = r'C:\Users\Gerun\PycharmProjects\291Internship\MAs\streaming.db'
def main():
    ...
    answer_dict = {}
    student_dict = {}
    load_student_answers(r"C:\Users\Gerun\PycharmProjects\291Internship\script_grader\student_answers\A1",student_dict,answer_dict,DB_FILE)
    while(True):
    ...
    with os.scandir(ans_path) as ans_dir:
        for entry in ans_dir:
            if entry.name.endswith(".txt") and entry.is_file():
                ...
                update_student_answers_json(entry,stud_dict,ans_dict,db_file)
    pprint.pprint(stud_dict)
def update_student_answers_json(stud_entry,stud_dict,key_json,db_file):
    id = stud_entry.name.split('_')[0]
    if id not in stud_dict:
        stud_dict[id] = {}
        stud_dict[id]['queries'] = {}
    parse_answers_txt(stud_entry.path,stud_dict)
...
def parse_answers_txt(file_path,stud_dict):
    '''
    ...
    with open(file_path,'r') as f:
        ...
        query = []
        for row in f:
            if first and re.match('ccid:',row.lower()):
                ccid = row.lower().split(':')[1].strip()
                print(row)
            elif re.match('Q[0-9]:',row):
                if first:
                    q_num = row.strip()[:2].lower()
                if not first:
                    stud_dict[ccid]['queries'][q_num] = ' '.join(query)
                    query = []
                    q_num = row.strip()[:2].lower()
                first = False
            elif row:
